...
def post_operate(img):
    ...
    for i in range(len(contours)):
        ...
        if cv2.contourArea(contours[i]) > thre_vol:
            print('after : ' , cv2.contourArea(contours[i]))
...
def test_img_save(test_imgs , test_masks , predict_masks , names , test_save_path):
    ...
    for i in range(0 , len(test_imgs)):
        ...
        # 对预测的结果进行处理
        p_mask[: , : , 0] , contours0 = post_operate(np.uint8(p_mask[: , : , 0]))
        p_mask[: , : , 1] , contours1 = post_operate(np.uint8(p_mask[: , : , 1]))
        ...
        p_mask = np.ones((256 , 256 , 3) , dtype = np.uint8)
        p_mask[: , : , 2] = 255
        p_mask = cv2.fillPoly(p_mask, contours0, (255 , 0 , 0))
        p_mask = cv2.fillPoly(p_mask, contours1, (0 , 255 , ))
...
if __name__ == '__main__':
    ...
    for weight in test_weights[5 : ]:
        ...
        test_imgs , test_masks , names = sr.generate_test_data(test_nums = 100)
